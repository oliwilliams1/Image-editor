...
uniform float u_Ct_s  = 6500.0;  // @min=2000 @max=10000 source color temperature @unit=K
uniform float u_Ct_t  = 6500.0;  // @min=2000 @max=10000 target color temperature @unit=K
...
void main()
{
	...
	vec3 wr = xyz2lms * cct2xyz(u_Ct_s);
    vec3 wt = xyz2lms * cct2xyz(u_Ct_t);

    vec3 lms = xyz2lms * rgb2xyz * pow(imageColour.rgb, vec3(gamma));

    lms.rg *= wt.rg / wr.rg;
    lms.b = wt.b * pow(lms.b / wr.b, pow(wr.b / wt.b, 0.0834));

    FragColor = vec4(pow(xyz2rgb * lms2xyz * lms, vec3(1.0 / gamma)), 1.0);
}